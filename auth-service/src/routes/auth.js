...
async function logEvent({ service='auth-service', level, event, userId, ip, method, path, statusCode, message, meta }) {
  try {
    const res = await fetch(`http://log-service:3003/api/logs/internal`, {
      method:  'POST',
    ...
    if (!res.ok) {
      console.error(`[AUTH] logEvent failed with status ${res.status}`);
    }
  } catch (err) {
    console.error(`[AUTH] logEvent error:`, err.message);
  }
}
